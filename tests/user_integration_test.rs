...
use mini_rust_api::features::user::{Email, User};
...
fn test_user_registration_and_authentication_flow() {
    ...
    let email =
        Email::new("john.doe@example.com".to_string()).expect("Valid email should be created");
...
fn test_user_email_change_flow() {
    let initial_email = Email::new("original@example.com".to_string()).unwrap();
    let mut user = User::register(
        initial_email,
    ...
    assert_eq!(user.email().as_str(), "original@example.com");

    // Change to a new email
    let new_email = Email::new("updated@example.com".to_string()).unwrap();
    user.change_email(new_email)
        .expect("Email change should succeed");

    assert_eq!(user.email().as_str(), "updated@example.com");

    // Changing to the same email should be a no-op (not an error)
    let same_email = Email::new("updated@example.com".to_string()).unwrap();
...
fn test_user_password_change_flow() {
    let email = Email::new("password.test@example.com".to_string()).unwrap();
    let mut user = User::register(
        email,
...
fn test_user_registration_validation_errors() {
    ...
    let email = Email::new("test2@example.com".to_string()).unwrap();
    ...
    let email = Email::new("test3@example.com".to_string()).unwrap();
    ...
    let email = Email::new("test4@example.com".to_string()).unwrap();
...
fn test_email_normalization_in_user_flow() {
    ...
    let email = Email::new("  JOHN.DOE@EXAMPLE.COM  ".to_string())
